...
const DRILLS: Record<string, { key: string; title: string; desc: string; yt?: string }[]> = {
  ...
  backFloat: [
    { key: "ear-water", title: "Ears-in Water", desc: "Relax head back; ears under water 10s" }
  ]
};
...
type SkillState = {
  status: Status;
  logs: { id: string; type: "attempt" | "achieved"; date: string; cohortId: string | null; notes: string }[];
  achievedAt?: string;
...
type Student = {
  ...
  practice: { id: string; skillKey: string; drillKey: string; title: string; due: string; notes: string; status: "Assigned" | "Done" }[];
...
const SAMPLE_STUDENTS: Student[] = [
  { id: genId(), name: "Lilly Johnson", notes: [], practice: [], cohortHistory: [], currentCohortId: null, skills: { ...defaultSkillMap(), waterComfort: { status: "Achieved", logs: [{ id: genId(), type: "achieved", date: new Date().toISOString(), cohortId: null, notes: "Comfortable in shallow end" }] }, backFloat: { status: "In Progress", logs: [] } } as any },
  { id: genId(), name: "Ryan Patel", notes: [], practice: [], cohortHistory: [], currentCohortId: null, skills: { ...defaultSkillMap(), flutterKick: { status: "Achieved", logs: [{ id: genId(), type: "achieved", date: new Date().toISOString(), cohortId: null, notes: "" }] }, freestyleArms: { status: "In Progress", logs: [] } } as any }
...
export default function App() {
  ...
  function addSkillLog(stuId: string, key: string, payload: SkillState["logs"][number]) {
    const base = Array.isArray(studentsRaw) ? studentsRaw : [];
    setStudents(
      base.map((s) => {
        if (s.id !== stuId) return s;
  ...
  function addPractice(stuId: string, { skillKey, drillKey, due, notes }: { skillKey: string; drillKey: string; due: string; notes: string }) {
    ...
    setStudents(
      base.map((s) => {
        ...
        const item = { id: genId(), skillKey, drillKey, title, due, notes: notes || "", status: "Assigned" as const };
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 16, marginTop: 16 }}>
          <Roster
            ...
            onRemove={selected ? () => removeStudent(selected.id) : undefined}
          />
            ...
            {!selected ? (
              ...
              <>
                ...
                {tab === "cohorts" && (
                  <div style={{ marginTop: 12 }}>
                    <Card>
                      ...
                      <ul style={{ marginTop: 8, paddingLeft: 16 }}>
                        {(!selected.cohortHistory || selected.cohortHistory.length === 0) && <li style={{ fontSize: 14, color: "#64748b" }}>No cohorts yet. Assign a current cohort above.</li>}
                        {selected.cohortHistory?.map((cid) => {
                          const c = cohortsSafe.find((x) => x.id === cid);
                          if (!c) return null;
                          return (
                            <li key={cid} style={{ border: "1px solid #e2e8f0", borderRadius: 8, padding: 8, marginTop: 8, fontSize: 14 }}>
                    ...
                    <Card style={{ marginTop: 12 }}>
                      <div style={{ fontWeight: 600, fontSize: 14 }}>Achievements by cohort</div>
                      <ul style={{ marginTop: 8, paddingLeft: 16 }}>
                        {ALL_SKILLS.map((sk) => {
                          const base = selected.skills && selected.skills[sk.key];
                          const ss = base ? { ...base, logs: Array.isArray(base.logs) ? base.logs : [] } : { status: "Not Started", logs: [] as any[] };
                          if (!ss.achievedAt) return null;
                          const c = cohortsSafe.find((x) => x.id === ss.achievedCohortId);
                          return (
                            <li key={sk.key} style={{ border: "1px solid #e2e8f0", borderRadius: 8, padding: 8, marginTop: 8, fontSize: 14 }}>
                              <div style={{ fontWeight: 600 }}>{sk.name}</div>
                              <div style={{ fontSize: 12, color: "#64748b" }}>
                                Achieved {new Date(ss.achievedAt).toLocaleDateString()} {c && (
                                  <>
                                    in <span style={{ background: "#f1f5f9", padding: "2px 6px", borderRadius: 6 }}>{c.name}</span>
                                  </>
                                )}
                              </div>
                            </li>
                ...
                {tab === "resources" && (
                  <div style={{ display: "grid", gap: 10, marginTop: 12 }}>
                    {ALL_SKILLS.map((sk) => (
                      <YTCard key={sk.key} title={sk.name} url={sk.yt} />
                    ))}
                  </div>
                )}
              </>
        ...
        <div style={{ marginTop: 16 }}>
          <Card>
            <h3 style={{ marginTop: 0 }}>Cohort manager</h3>
            {cohortsSafe.length === 0 && <div style={{ fontSize: 14, color: "#64748b" }}>No cohorts yet. Click "Add cohort" above.</div>}
            <div style={{ display: "grid", gap: 10, gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))" }}>
              {cohortsSafe.map((c) => (
                <div key={c.id} style={{ border: "1px solid #e2e8f0", borderRadius: 10, padding: 10, fontSize: 14 }}>
                  <div style={{ fontWeight: 600 }}>{c.name}</div>
                  <div style={{ fontSize: 12, color: "#64748b" }}>
                    {new Date(c.start).toLocaleDateString()} → {new Date(c.end).toLocaleDateString()}
                  </div>
        ...
        <div style={{ marginTop: 16, textAlign: "center", fontSize: 12, color: "#64748b" }}>Prototype – data is saved only in your browser. Import/Export to back it up.</div>
      </div>
    </div>
  );
}

function Roster({ students, selectedId, onSelect, onRemove }: { students: Student[]; selectedId: string | null; onSelect: (id: string) => void; onRemove?: () => void }) {
  return (
    <div style={{ border: "1px solid #e2e8f0", borderRadius: 12, padding: 12 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
      ...
      <div style={{ marginTop: 8 }}>
        {students.length === 0 && <div style={{ fontSize: 14, color: "#64748b" }}>No students found.</div>}
        {students.map((s) => {
          const level = computeLevel(s.skills);
          const isSelected = s.id === selectedId;
          return (
            <button key={s.id} onClick={() => onSelect(s.id)} style={{ display: "block", width: "100%", textAlign: "left", border: "1px solid #e2e8f0", borderRadius: 10, padding: 10, marginTop: 8, background: isSelected ? "#eef2ff" : "white" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div>
      ...
      {onRemove && (
        <div style={{ marginTop: 8, textAlign: "right" }}>
          <button onClick={onRemove} style={{ background: "#fee2e2" }}>
            Remove selected
          </button>
        </div>
      )}
...
function AddNote({ onAdd }: { onAdd: (text: string) => void }) {
  ...
  return (
    <div style={{ border: "1px solid #e2e8f0", borderRadius: 10, padding: 10 }}>
      <div style={{ fontWeight: 600, fontSize: 14, marginBottom: 6 }}>Add lesson note</div>
      <textarea value={text} onChange={(e) => setText(e.target.value)} placeholder="What was practiced? Wins? What needs reinforcement?" style={{ width: "100%", height: 96 }} />
...
function YTCard({ title, url }: { title: string; url?: string }) {
  ...
  return (
    <div style={{ border: "1px solid #e2e8f0", borderRadius: 10, padding: 10 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <button onClick={() => setOpen((o) => !o)}>{open ? "Hide" : "Play"}</button>
      </div>
      {url && (
        <a href={url} target="_blank" rel="noreferrer" style={{ fontSize: 14, textDecoration: "underline" }}>
          Open on YouTube
        </a>
      )}
      {open && !embedError && url && (
      ...
      {open && embedError && <div style={{ marginTop: 8, fontSize: 14, color: "#b45309", background: "#fffbeb", padding: 8, borderRadius: 8 }}>Embed blocked by browser security. Use the link above to watch on YouTube.</div>}
    </div>
  );
}
